...
{
	@Query(value="SELECT * FROM login WHERE farmer_name= :name and farmer_password= :password", nativeQuery=true)
//	@Query(value="SELECT  FROM login l WHERE l.farmer_name= :name AND l.farmer_password=:password", nativeQuery=true)
	Login checkUser(@Param(value="name") String name,@Param(value="password") String password);
}
